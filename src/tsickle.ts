...
export function emitWithTsickle(
  ...
  const tsTransformers: ts.CustomTransformers = {
    before: [
      ...normalizeArray(customTransformers.beforeTsickle),
      ...normalizeArray(tsickleTransformers.before)
          .map(tf => skipTransformForSourceFileIfNeeded(host, tf)),
      ...normalizeArray(customTransformers.beforeTs),
    ],
    after: [
      ...normalizeArray(customTransformers.afterTs),
      ...normalizeArray(tsickleTransformers.after)
          .map(tf => skipTransformForSourceFileIfNeeded(host, tf))
    ]
  };
