...
function emitWithTsickle(program, host, tsHost, tsOptions, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers) {
    ...
    var tsTransformers = {
        before: __spread(normalizeArray(customTransformers.beforeTsickle), normalizeArray(tsickleTransformers.before)
            .map(function (tf) { return skipTransformForSourceFileIfNeeded(host, tf); }), normalizeArray(customTransformers.beforeTs)),
        after: __spread(normalizeArray(customTransformers.afterTs), normalizeArray(tsickleTransformers.after)
            .map(function (tf) { return skipTransformForSourceFileIfNeeded(host, tf); }))
    };
